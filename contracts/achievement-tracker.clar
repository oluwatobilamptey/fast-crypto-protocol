;; achievement-tracker
;; Part of the Fast Crypto Protocol
;; 
...
(define-data-var protocol-achievement-counter uint u1)
...
(define-private (generate-goal-id)
  (let ((current-id (var-get protocol-achievement-counter)))
    (var-set protocol-achievement-counter (+ current-id u1))
    current-id
  )
)
